...
//  implicit val materializer = FlowMaterializer()
  ...
  def mainFlow() = {
    ...
    actorSystem.scheduler.schedule(
      Duration.Zero,
      Duration(CYCLE_DURATION, TimeUnit.SECONDS),
      actor,
      Start
    )
  ...
  def startCycle(cycle: Int) = {
    ...
    LogSource.deleteLogsBeforeCycle(
      cycle - com.patson.model.Log.RETENTION_CYCLE
    )

    // purge history
    println("Purging link history")
    ChangeHistorySource.deleteLinkChangeByCriteria(
      List(("cycle", "<", cycle - 500))
    )
    ...
    println(
      "cycle " + cycle + " spent " + (cycleEnd - cycleStartTime) / 1000 + " secs"
    )
    cycleEnd
  }
  ...
  def postCycle(currentCycle: Int) = {
    ...
    OilSimulation.simulate(
      currentCycle
    ) // simulate at the beginning of a new cycle
    println("Loan simulation")
    LoanInterestRateSimulation.simulate(
      currentCycle
    ) // simulate at the beginning of a new cycle
    // refresh delegates
  ...
  /** The simulation can be seen like this: On week(cycle) n. It starts the long
    * simulation (pax simulation) at the "END of the week" when it finishes
    * computing the pax of the past week. It sets the current week to next week
    * (which indicates a beginning of week n + 1) It then runs some postCycle
    * task (these tasks should be short and can be regarded as things to do at
  ...
  class MainSimulationActor extends Actor {
    ...
    def receive = { case Start =>
      ...
      postCycle(
        currentWeek
      ) // post cycle do some quick updates, no long simulation

      // notify the websockets via EventStream
      println("Publish Cycle Complete message")
      SimulationEventStream.publish(
        CycleCompleted(currentWeek - 1, endTime),
        None
      )
    }
  }
