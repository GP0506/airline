...
import com.patson.data.{
  ...
  CycleSource,
  EventSource,
  LinkStatisticsSource
}
import com.patson.model.event.{
  EventType,
  Olympics,
  OlympicsAirlineVote,
  OlympicsAirlineVoteWithWeight,
  OlympicsVoteRound
}
import com.patson.model.{
  ...
  AirportFeatureType,
  Computation,
  OlympicsInProgressFeature,
  OlympicsPreparationsFeature
}
...
object EventSimulation {
  ...
  def simulate(cycle: Int): Unit = {
    // purge
    EventSource.deleteEventsBeforeCycle(
      CycleSource.loadCycle() - MAX_HISTORY_DURATION
    )
    val events = EventSource.loadEvents().sortBy(_.startCycle).reverse

    val currentOlympicsOption = events.find(
      _.eventType == EventType.OLYMPICS
    ) map (_.asInstanceOf[Olympics])
    simulateOlympics(cycle, currentOlympicsOption)
  }

  def simulateOlympics(
      cycle: Int,
      currentOlympicsOption: Option[Olympics]
  ): Unit = {
    currentOlympicsOption.foreach { currentOlympics =>
      if (
        !currentOlympics.isActive(cycle) && currentOlympics.isActive(cycle - 1)
      ) { // just finished last turn
        simulateOlympicsEnding(currentOlympics)
      }
    }

    val createNewOlympics =
      currentOlympicsOption.isEmpty || !currentOlympicsOption.get.isActive(
        cycle
      )
      ...
      olympics.currentYear(cycle) match {
        case 1 =>
          ...
          val affectedAirports = candidates.map { principalAirport =>
            (
              principalAirport,
              simulateOlympicsAffectedAirport(principalAirport)
            )
          }.toMap

          println(s"Olympics airport(s) within radius: $affectedAirports")
          EventSource.saveOlympicsAffectedAirports(
            olympics.id,
            affectedAirports
          )
        ...
        case 2 =>
          ...
          if (voteRounds.size > 0) {
            val selectedAirport =
              voteRounds.last.votes.toList.sortBy(_._2).last._1
          ...
          Olympics.getSelectedAffectedAirports(olympics.id).foreach { airport =>
            AirportSource
              .saveAirportFeature(airport.id, OlympicsPreparationsFeature(1))
          }
        ...
        case 4 =>
          ...
          Olympics.getSelectedAffectedAirports(olympics.id).foreach { airport =>
            AirportSource.deleteAirportFeature(
              airport.id,
              AirportFeatureType.OLYMPICS_PREPARATIONS
            )
            AirportSource
              .saveAirportFeature(airport.id, OlympicsInProgressFeature(1))
          }
      }
    }
  ...
  val CANDIDATE_MIN_POPULATION = 700000
  ...
  def selectCandidates(allAirports: List[Airport]): List[Airport] = {
    val previousOlympics = EventSource
      .loadEvents()
      .filter(_.eventType == EventType.OLYMPICS)
      .sortBy(_.startCycle)
      .dropRight(1) // drop the current one

    val cooldownCountries: List[String] = previousOlympics
      .takeRight(HOST_COUNTRY_COOLDOWN)
      .map(_.asInstanceOf[Olympics])
      .map { olympics =>
    ...
    val cooldownZones: List[String] = previousOlympics
      .takeRight(HOST_ZONE_COOLDOWN)
      .map(_.asInstanceOf[Olympics])
      .map { olympics =>
  ...
  def simulateOlympicsVoteRounds(
      olympics: Olympics
  ): List[OlympicsVoteRound] = {
    val voteWeights = Olympics
      .getVoteWeights()
      .view
      .map { case (airline, weight) =>
        (airline.id, weight)
      }
      .toMap

    val airlineVotes: Map[Airline, OlympicsAirlineVoteWithWeight] = EventSource
      .loadOlympicsAirlineVotes(olympics.id)
      .view
      .mapValues { airlineVote =>
        airlineVote.withWeight(voteWeights.getOrElse(airlineVote.airline.id, 0))
      }
      .toMap
    val candidates: List[Airport] =
      EventSource.loadOlympicsCandidates(olympics.id)
    simulateOlympicsVoteRounds(candidates, airlineVotes)
  }

  def simulateOlympicsVoteRounds(
      candidates: List[Airport],
      airlineVotes: Map[Airline, OlympicsAirlineVoteWithWeight]
  ): List[OlympicsVoteRound] = {
    ...
    while (remainingCandidates.size >= 1) {
      ...
      airlineVotes.foreach { case (airline, airlineVote) =>
        airlineVote.voteList.find(votedAirport =>
          remainingCandidates.contains(votedAirport)
        ) match { // go down by the list (highest precedence first), find the first one that is still in the candidates list
          case Some(legitVotedAirport) =>
            val currentVotesForThisAirport = votesByAirport(legitVotedAirport)
            votesByAirport.put(
              legitVotedAirport,
              currentVotesForThisAirport + airlineVote.weight
            )
      ...
      val resultOfThisRound =
        OlympicsVoteRound(round = voteRound, votes = votesByAirport.toMap)
      voteRoundResults.append(resultOfThisRound)

      // eliminate the candidate with least votes
      val evictingCandidate: Airport = votesByAirport.toList
        .sortBy(_._1.id)
        .sortBy(_._2)
        .apply(0)
        ._1 // sort by airport Id first for more predictable result
  ...
  def simulateOlympicsPassengerGoals(olympics: Olympics) = {
    ...
    val passengersByAirline: MapView[Airline, Int] = allLinkStats
      .groupBy(_.key.airline)
      .view
    ...
    for (i <- 0 until Olympics.WEEKS_PER_YEAR) {
      olympicsTotalPassengers += Olympics.getDemandMultiplier(
        i
      ) * DemandGenerator.OLYMPICS_DEMAND_BASE
    }

    val passengerGoalByAirline = passengersByAirline.mapValues { passengers =>
      val goal =
        (passengers.toDouble / totalPassengers * olympicsTotalPassengers * GOAL_BASE_FACTOR).toInt
  ...
  def simulateOlympicsEnding(olympics: Olympics) = {
    Olympics.getSelectedAffectedAirports(olympics.id).foreach { airport =>
      AirportSource.deleteAirportFeature(
        airport.id,
        AirportFeatureType.OLYMPICS_IN_PROGRESS
      )
    }
  }
