...
object AirlineSimulation {
  ...
  val BANKRUPTCY_CASH_THRESHOLD = -50000000 // -10M

  def airlineSimulation(
      cycle: Int,
      flightLinkResult: List[LinkConsumptionDetails],
      loungeResult: scala.collection.immutable.Map[
        Lounge,
    ...
    val allFlightLinksByAirlineId = allLinks
      .filter(_.transportType == TransportType.FLIGHT)
      .map(_.asInstanceOf[Link])
      .groupBy(_.airline.id)
    ...
    val allTransactions =
      AirlineSource.loadTransactions(cycle).groupBy { _.airlineId }
    val allTransactionalCashFlowItems
        : scala.collection.immutable.Map[Int, List[AirlineCashFlowItem]] =
      AirlineSource.loadCashFlowItems(cycle).groupBy { _.airlineId }
    ...
    val allCountries = CountrySource
      .loadAllCountries()
      .map(country => (country.countryCode, country))
      .toMap

    val loungesByAirlineId =
      scala.collection.mutable.Map[Int, ListBuffer[Lounge]]()
    AirlineSource.loadAllLounges.foreach(lounge =>
      loungesByAirlineId.getOrElseUpdate(
        lounge.airline.id,
        ListBuffer[Lounge]()
      ) += lounge
    )
    ...
    val allCashFlows =
      ListBuffer[AirlineCashFlow]() // cash flow for accounting purpose
    ...
    val airportChampionsByAirlineId
        : immutable.Map[Int, List[AirportChampionInfo]] =
      ChampionUtil.loadAirportChampionInfo().groupBy(_.loyalist.airline.id)
    val titlesByCountryCodeAndAirlineId
        : immutable.Map[(String, Int), List[CountryAirlineTitle]] =
      CountrySource
        .loadCountryAirlineTitlesByCriteria(List.empty)
        .groupBy(entry =>
          (entry.country.countryCode, entry.airline.id)
        ) // key is (CountryCode, AirlineId)
    val cashFlows = Map[Airline, Long]() // cash flow for actual deduction

    val alliances = AllianceSource.loadAllAlliances()
    val allianceByAirlineId: scala.collection.immutable.Map[Int, Alliance] =
      alliances.flatMap { alliance =>
        (alliance.members
          .filter(_.role != AllianceRole.APPLICANT)
          .map(member => (member.airline.id, alliance)))
      }.toMap
    val allianceRankings = Alliance.getRankings(alliances)

    val fuelContractsByAirlineId =
      OilSource.loadAllOilContracts().groupBy(contract => contract.airline.id)
    val fuelInventoryPolicyByAirlineId = OilSource.loadAllOilInventoryPolicies
      .map(policy => (policy.airline.id, policy))
      .toMap
    ...
    allAirlines.foreach { airline =>
      ...
      val linksIncome = flightLinkResultByAirline.get(airline.id) match {
        case Some(linkConsumptions) => {
          ...
          val linksInflightCost =
            linkConsumptions.foldLeft(0L)(_ + _.inflightCost)
          val linksDelayCompensation =
            linkConsumptions.foldLeft(0L)(_ + _.delayCompensation)
          val linksMaintenanceCost =
            linkConsumptions.foldLeft(0L)(_ + _.maintenanceCost)
          ...
          val linksExpense =
            linksAirportFee + linksCrewCost + linksFuelCost + linksInflightCost + linksDelayCompensation + linksMaintenanceCost + linksDepreciation + linksLoungeCost
          ...
          LinksIncome(
            ...
            expense = linksExpense,
            ticketRevenue = linksRevenue,
            airportFee = -1 * linksAirportFee,
            fuelCost = -1 * linksFuelCost,
            crewCost = -1 * linksCrewCost,
            inflightCost = -1 * linksInflightCost,
            delayCompensation = -1 * linksDelayCompensation,
            maintenanceCost = -1 * linksMaintenanceCost,
            loungeCost = -1 * linksLoungeCost,
            depreciation = -1 * linksDepreciation,
            cycle = currentCycle
          )
        }
        case None =>
      ...
      val transactionsIncome = allTransactions.get(airline.id) match {
        case Some(transactions) => {
          ...
          transactions.foreach { transaction =>
            ...
            val existingAmount =
              summary.getOrElse(transaction.transactionType, 0L)
            summary.put(
          ...
          TransactionsIncome(
            ...
            revenue,
            expense,
            capitalGain = summary.getOrElse(TransactionType.CAPITAL_GAIN, 0),
            createLink = summary.getOrElse(TransactionType.CREATE_LINK, 0),
            cycle = currentCycle
          )
        }
        case None =>
      ...
      val linksOfThisAirline =
        allFlightLinksByAirlineId.getOrElse(airline.id, List.empty)
      var serviceFunding =
        getServiceFunding(airline.getTargetServiceQuality(), linksOfThisAirline)
      ...
      airline.setCurrentServiceQuality(
        getNewQuality(currentServiceQuality, targetServiceQuality)
      )

      othersSummary.put(
        OtherIncomeItemType.SERVICE_INVESTMENT,
        serviceFunding * -1
      )
      ...
      othersSummary.put(
        OtherIncomeItemType.BASE_UPKEEP,
        -1 * baseUpkeep
      ) // negative number
      totalCashExpense += baseUpkeep

      // overtime compensation
      val linksByFromAirportId = allFlightLinksByAirlineId
        .get(airline.id)
      ...
      airline.bases.foreach { base =>
        ...
        val titleOption: Option[Title.Value] =
          titlesByCountryCodeAndAirlineId.get(
            (base.airport.countryCode, airline.id)
          ) match {
            case Some(titles) =>
              if (titles.length > 0) Some(titles(0).title)
              else None // now only 1 title per airline per country
            case None => None
          }
        ...
        if (compensationOfThisBase > 0) {
          println(
            s"${airline.name} Overtime compensation $compensationOfThisBase : capacity $staffCapacity ; required $staffRequired"
          )
        }
      ...
      othersSummary.put(
        OtherIncomeItemType.OVERTIME_COMPENSATION,
        -1 * overtimeCompensation
      ) // negative number
      totalCashExpense += overtimeCompensation

      val allAirplanesDepreciation =
        airplanesByAirline.getOrElse(airline.id, List.empty).foldLeft(0L) {
          case (depreciation, airplane) => (
            depreciation + airplane.depreciationRate
          )
        }

      val unassignedAirplanesDepreciation =
        allAirplanesDepreciation - linksDepreciation // account depreciation on planes that are without assigned links
      othersSummary.put(
        OtherIncomeItemType.DEPRECIATION,
        -1 * unassignedAirplanesDepreciation
      ) // not a cash expense

      val (loanPayment, interestPayment) = updateLoans(
        airline,
        currentCycle + 1
      ) // have to plus one here, as this is supposed to be done postcycle, but for accounting purpose we have to put it here
      ...
      loungeResult.filter(_._1.airline.id == airline.id).map {
        case (
              _,
              LoungeConsumptionDetails(_, selfVisitors, allianceVisitors, _)
            ) => {
      ...
      val actualFuelCost = fuelContractsByAirlineId.get(airline.id) match {
        case Some(contracts) =>
          val totalPaymentFromContract = contracts.map { contract =>
            oilConsumptionEntries += OilConsumptionHistory(
              airline,
              contract.contractPrice,
              contract.volume,
              OilConsumptionType.CONTRACT,
              currentCycle
            )
          ...
          if (totalVolumeFromContract <= barrelsUsed) {
            ...
            val inventoryPolicy = fuelInventoryPolicyByAirlineId.getOrElse(
              airline.id,
              OilInventoryPolicy.getDefaultPolicy(airline)
            )
            val inventoryPrice =
              inventoryPolicy.inventoryPrice(currentFuelPrice)
            val volumeFromInventory = barrelsUsed - totalVolumeFromContract
            val consumptionType =
              if (inventoryPolicy.factor == 0) OilConsumptionType.MARKET
              else OilConsumptionType.INVENTORY
...
//        println("barrels used: " + barrelsUsed + " acc. fuel cost " + accountingFuelCost + " actual fuel cost " + actualFuelCost.toLong + " profit " + fuelProfit)
      ...
      // campaign/ads
      val advertisementCost =
        DelegateSource.loadCampaignTasksByAirlineId(airline.id).map(_.cost).sum
      if (advertisementCost > 0) {
        totalCashExpense += advertisementCost
      }
      othersSummary.put(
        OtherIncomeItemType.ADVERTISEMENT,
        advertisementCost * -1
      )
      ...
      val othersIncome = OthersIncome(
        airline.id,
        othersRevenue - othersExpense,
        othersRevenue,
        othersExpense,
        loanInterest =
          othersSummary.getOrElse(OtherIncomeItemType.LOAN_INTEREST, 0),
        baseUpkeep =
          othersSummary.getOrElse(OtherIncomeItemType.BASE_UPKEEP, 0),
        overtimeCompensation =
          othersSummary.getOrElse(OtherIncomeItemType.OVERTIME_COMPENSATION, 0),
        serviceInvestment =
          othersSummary.getOrElse(OtherIncomeItemType.SERVICE_INVESTMENT, 0),
        maintenanceInvestment = othersSummary
          .getOrElse(OtherIncomeItemType.MAINTENANCE_INVESTMENT, 0),
        advertisement =
          othersSummary.getOrElse(OtherIncomeItemType.ADVERTISEMENT, 0),
        loungeUpkeep =
          othersSummary.getOrElse(OtherIncomeItemType.LOUNGE_UPKEEP, 0),
        loungeCost =
          othersSummary.getOrElse(OtherIncomeItemType.LOUNGE_COST, 0),
        loungeIncome =
          othersSummary.getOrElse(OtherIncomeItemType.LOUNGE_INCOME, 0),
        shuttleCost =
          othersSummary.getOrElse(OtherIncomeItemType.SHUTTLE_COST, 0),
        fuelProfit =
          othersSummary.getOrElse(OtherIncomeItemType.FUEL_PROFIT, 0),
        depreciation =
          othersSummary.getOrElse(OtherIncomeItemType.DEPRECIATION, 0),
        cycle = currentCycle
      )

      val airlineRevenue =
        linksIncome.revenue + transactionsIncome.revenue + othersIncome.revenue
      val airlineExpense =
        linksIncome.expense + transactionsIncome.expense + othersIncome.expense
      val airlineProfit = airlineRevenue - airlineExpense
      val airlineWeeklyIncome = AirlineIncome(
        airline.id,
        airlineProfit,
        airlineRevenue,
        airlineExpense,
      ...
      val operationCashFlow =
        totalCashFlow + loanPayment // exclude both interest and principle here, which WAS included in the total cash flow
      cashFlows.put(
        airline,
        totalCashFlow
      ) // this is week end flow, used for actual adjustment
      ...
      val transactionalCashFlowItems
          : scala.collection.immutable.Map[CashFlowType.Value, Long] =
        allTransactionalCashFlowItems.get(airline.id) match {
          case Some(items) =>
            items
              .groupBy(_.cashFlowType)
              .view
              .mapValues(itemsByType => itemsByType.map(_.amount).sum)
              .toMap
          case None => scala.collection.immutable.Map.empty
        }

      // include cash flow during the week, only use for accounting purpose here
      val baseConstruction =
        transactionalCashFlowItems.getOrElse(CashFlowType.BASE_CONSTRUCTION, 0L)
      val buyAirplane =
        transactionalCashFlowItems.getOrElse(CashFlowType.BUY_AIRPLANE, 0L)
      val sellAirplane =
        transactionalCashFlowItems.getOrElse(CashFlowType.SELL_AIRPLANE, 0L)
      val createLink =
        transactionalCashFlowItems.getOrElse(CashFlowType.CREATE_LINK, 0L)
      val facilityConstruction = transactionalCashFlowItems.getOrElse(
        CashFlowType.FACILITY_CONSTRUCTION,
        0L
      )
      val oilContract =
        transactionalCashFlowItems.getOrElse(CashFlowType.OIL_CONTRACT, 0L)

      val accountingCashFlow =
        totalCashFlow + baseConstruction + buyAirplane + sellAirplane + createLink + facilityConstruction + oilContract

      val loanPrincipal = loanPayment - interestPayment
      val airlineWeeklyCashFlow = AirlineCashFlow(
        airline.id,
        cashFlow = accountingCashFlow,
        operation = operationCashFlow,
        loanInterest = interestPayment * -1,
        loanPrincipal = loanPrincipal * -1,
        baseConstruction = baseConstruction,
        buyAirplane = buyAirplane,
        sellAirplane = sellAirplane,
        createLink = createLink,
        facilityConstruction = facilityConstruction,
        oilContract = oilContract,
        cycle = currentCycle
      )
      ...
      val reputationByPassengers =
        flightLinkResultByAirline.get(airline.id) match {
          case Some(linkConsumptions) =>
            val totalPassengerKilometers = linkConsumptions.foldLeft(0L) {
              (foldLong, linkConsumption) =>
      ...
      reputationBreakdowns.append(
        ReputationBreakdown(
          ReputationType.FLIGHT_PASSENGERS,
          reputationByPassengers
        )
      )
...
//        }
      ...
      val reputationByAirportChampions =
        airportChampionsByAirlineId.get(airline.id) match {
          case Some(airportChampions) =>
            airportChampions.map(_.reputationBoost).sum
          case None => 0
        }
      reputationBreakdowns.append(
        ReputationBreakdown(
          ReputationType.AIRPORT_LOYALIST_RANKING,
          reputationByAirportChampions
        )
      )

      val reputationBonusFromAlliance: Double =
        allianceByAirlineId.get(airline.id) match {
          case Some(alliance) =>
            allianceRankings.get(alliance) match {
      ...
      reputationBreakdowns.append(
        ReputationBreakdown(
          ReputationType.ALLIANCE_BONUS,
          reputationBonusFromAlliance
        )
      )
      ...
      if (
        targetReputation > currentReputation && targetReputation - currentReputation > MAX_REPUTATION_DELTA
      ) {
        targetReputation = currentReputation + MAX_REPUTATION_DELTA
      } else if (
        targetReputation < currentReputation && currentReputation - targetReputation > MAX_REPUTATION_DELTA
      ) {
        targetReputation = currentReputation - MAX_REPUTATION_DELTA
      }
      ...
      if (airline.getBalance() < BANKRUPTCY_CASH_THRESHOLD) {
        ...
        if (shouldReset) {
          ...
          println(
            s"Resetting $airline due to negative cash and unprofitable flights"
          )
      ...
      println(
        airline + " profit is: " + airlineProfit + " existing balance (not updated yet) " + airline
          .getBalance() + " reputation " + airline
          .getReputation() + " cash flow " + totalCashFlow
      )
    }

    AirlineSource.saveAirlinesInfo(allAirlines)
    cashFlows
      .foreach { // for balance it's safer to use adjust instead of setting it directly
        case (airline, cashFlow) =>
          AirlineSource.adjustAirlineBalance(airline.id, cashFlow)
      }
...
//  }
  ...
  /** compute monthly and yearly income
    ...
    */
  def computeAccumulateIncome(
      weeklyIncome: AirlineIncome
  ): List[AirlineIncome] = {
    // get existing entry
    val currentWeek = MainSimulation.currentWeek
    val airlineId = weeklyIncome.airlineId
    val currentMonthIncomeOption =
      if (currentWeek % 4 == 0) None
      else
        IncomeSource.loadIncomeByAirline(
          airlineId,
          currentWeek - 1,
          Period.MONTHLY
        )
    ...
    val currentYearIncomeOption =
      if (currentWeek % 52 == 0) None
      else
        IncomeSource.loadIncomeByAirline(
          airlineId,
          currentWeek - 1,
  ...
  /** compute monthly and yearly cash flow
    ...
    */
  def computeAccumulateCashFlow(
      weeklyCashFlow: AirlineCashFlow
  ): List[AirlineCashFlow] = {
    // get existing entry
    val currentWeek = MainSimulation.currentWeek
    val airlineId = weeklyCashFlow.airlineId
    val currentMonthCashFlowOption =
      if (currentWeek % 4 == 0) None
      else
        CashFlowSource.loadCashFlowByAirline(
          airlineId,
          currentWeek - 1,
          Period.MONTHLY
        )
...
//  }
  ...
  def getServiceFunding(targetQuality: Int, links: List[Link]): Long = {
    val totalPassengerMileCapacity = links.map { link =>
      link.frequency * link
        .getAssignedModel()
        .fold(0L)(_.capacity.toLong) * link.distance
    }.sum
    getServiceFunding(targetQuality, totalPassengerMileCapacity)
  }

  val getServiceFunding: (Int, Long) => Long =
    (targetQuality: Int, totalPassengerMileCapacity: Long) => {
      val MIN_PASSENGER_MILE_CAPACITY = 1000 * 1000
      val passengerMileCapacity = Math
        .max(totalPassengerMileCapacity, MIN_PASSENGER_MILE_CAPACITY)
        .toDouble

      val funding = Math.pow(
        targetQuality.toDouble / 40,
        2.5
      ) * (passengerMileCapacity / 4000) * 30
      funding.toLong
    }

  val getNewQuality: (Double, Double) => Double =
