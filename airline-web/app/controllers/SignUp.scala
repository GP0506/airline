...
class SignUp @Inject() (cc: ControllerComponents)(ws: WSClient)
  ...
  /** Sign Up Form definition.
    *
    * Once defined it handle automatically, , validation, submission, errors,
    * redisplaying, ...
    */
  val signupForm: Form[NewUser] = Form(
    // Define a mapping that will handle User values
    mapping(
      "username" -> text(minLength = 4, maxLength = 20)
        .verifying(
          "username can only contain alphanumeric characters",
          userName =>
            userName.forall(char => char.isLetterOrDigit && char <= 'z')
        )
        .verifying(
          "This username is not available",
          userName =>
            !UserSource
              .loadUsersByCriteria(List.empty)
              .map { _.userName.toLowerCase() }
      ...
      "airlineName" -> text(minLength = 1, maxLength = 50)
        .verifying(
          "Airline name can only contain space and characters",
          airlineName =>
            airlineName.forall(char =>
              (char.isLetter && char <= 'z') || char == ' '
            ) && !"".equals(airlineName.trim())
        )
        .verifying(
          "This airline name is not available",
          airlineName =>
            !AirlineSource
              .loadAllAirlines(false)
              .map { _.name.toLowerCase().replaceAll("\\s", "") }
              .contains(airlineName.replaceAll("\\s", "").toLowerCase())
        )
    )
    ...
    // so we have to define custom binding/unbinding functions
    {
      ...
      (username, email, passwords, airlineName) =>
        NewUser(username.trim, passwords._1, email.trim, airlineName.trim)
    } {
      // Unbinding: Create the mapping values from an existing User value
      user =>
        Some(user.username, user.email, (user.password, ""), user.airlineName)
    }
  )
