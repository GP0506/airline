...
@Singleton
class AccountApplication @Inject() (cc: ControllerComponents)
...
//    EmailUtil.sendEmail("patson_luk@hotmail.com", "info@myfly.club", "testing", "testing");
//    Ok(Json.obj())
//  }
  ...
  def forgotIdSubmit = Action { implicit request =>
    forgotIdForm.bindFromRequest.fold(
      ...
      userInput => {
        val users =
          UserSource.loadUsersByCriteria(List(("email", userInput.email)))
        if (users.size > 0) {
          println("Sending email for forgot ID " + users)
          EmailUtil.sendEmail(
            userInput.email,
            fromEmail,
            "Forgot User Name from myfly.club",
            getForgotIdMessage(users)
          )
        } else {
          println(
            "Sending email for forgot ID but email " + userInput.email + " has no account!"
          )
        }
        Ok(html.checkEmail())
      }
      ...
      userInput => {
        ...
        if (user.email == userInput.email) {
          ...
          EmailUtil.sendEmail(
            user.email,
            fromEmail,
            "Reset password for myfly.club",
            getResetPasswordMessage(user, baseUrl)
          )
        } else {
          println(
            "Want to reset password for " + user.userName + " but email does not match!"
          )
        }
  ...
  def getForgotIdMessage(users: List[User]) = {
    val message = new StringBuilder(
      "Your User Name(s) registered with this email address : \r\n"
    )
    users.foreach { user =>
