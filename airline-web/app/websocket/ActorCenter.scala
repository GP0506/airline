...
sealed class LocalActor(out: ActorRef, airlineId: Int) extends Actor {
  ...
  def receive = {
    ...
    case (topic: SimulationEvent, payload: Any) =>
      Some(topic).collect {
        case CycleCompleted(cycle, cycleEndTime) =>
          ...
          out ! Json.obj(
            "messageType" -> "cycleCompleted",
            "cycle" -> cycle
          ) // if a CycleCompleted is published to the stream, notify the out(websocket) of the cycle
        ...
        case CycleInfo(cycle, fraction, cycleDurationEstimation) =>
          ...
          out ! Json.obj(
            "messageType" -> "cycleInfo",
            "cycle" -> cycle,
            "fraction" -> fraction,
            "cycleDurationEstimation" -> cycleDurationEstimation
          )
      }
    ...
    case AirlinePrompts(_, prompts) =>
      ...
      prompts.tutorials.foreach { case AirlineTutorial(airline, tutorial) =>
    ...
    case AirlinePendingActions(airline, pendingActions: List[PendingAction]) =>
      ...
      out ! Json.obj(
        "messageType" -> "pendingAction",
        "actions" -> Json.toJson(pendingActions.map(_.category.toString))
      )
    case any =>
  ...
  override def postStop() = {
    ...
    println(
      self.path.toString + " stopped (post stop), unsubscribed from all event streams"
    )
  }
}

class ResetTask(localActor: ActorRef, remoteActor: ActorSelection)
    extends TimerTask {
...
//  context.watch(broadcastActor)
  ...
  override def receive = {
    case (topic: SimulationEvent, payload: Any) =>
      println(
        s"Local main actor received topic $topic, re-publishing to ${actorSystem}"
      )
      Some(topic).collect { case CycleCompleted(cycle, cycleEndTime) =>
        ...
        // GooglePhotoUtil.refreshBanners() // causes java.lang.NullPointerException
      ...
      actorSystem.eventStream.publish(
        topic,
        payload
      ) // relay to local event stream... since i don't know if I can subscribe to remote event stream...
...
sealed class ReconnectActor(remoteActor: ActorSelection) extends Actor {
  ...
  override def receive = {
    case lifeCycleEvent: DisassociatedEvent => {
      if (!disconnected) {
        println(
          s"Disassociated. Start pinging the remote actor! from reconnect actor $this"
        )
        disconnected = true
...
object ActorCenter {
  ...
  val actorHost =
    if (configFactory.hasPath("airline.akka-actor.host"))
      configFactory.getString("airline.akka-actor.host")
    else "127.0.0.1:2552"
  println("!!!!!!!!!!!!!!!AKK ACTOR HOST IS " + actorHost)

  val subscribers = mutable.HashSet[ActorRef]()
  val remoteMainActor = system.actorSelection(
    "akka.tcp://" + REMOTE_SYSTEM_NAME + "@" + actorHost + "/user/" + BRIDGE_ACTOR_NAME
  )
  val localMainActor = system.actorOf(
    Props(classOf[LocalMainActor], remoteMainActor),
    "local-main-actor"
  )

  val reconnectActor = system.actorOf(
    Props(classOf[ReconnectActor], remoteMainActor),
    "reconnect-actor"
  )
